...
exports.updateAccessToken = (0, catchAsyncErrors_1.catchAsyncError)(async (req, res, next) => {
    try {
        ...
        const message = "Could not fresh token";
        if (!decoded) {
        ...
        await redis_1.redis.set(user._id, JSON.stringify(user), "EX", 604800); // (604800)it means => 7days
        // res.status(200).json({
        //   success: "success",
        //   accessToken,
        // });
...
exports.updateUserRole = (0, catchAsyncErrors_1.catchAsyncError)(async (req, res, next) => {
    try {
        ...
        const user = await user_model_1.default.findOneAndUpdate({ email: email }, { role: role }, { new: true });
        res.status(201).json({
            success: true,
